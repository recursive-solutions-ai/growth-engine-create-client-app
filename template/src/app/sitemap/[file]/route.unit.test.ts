...
describe('GET /sitemap/[file] (batches)', () => {
	...
	async function loadRoute(opts: {
		...
		landingEntries?: Array<{ url: string }>
		topicEntries?: Array<{ url: string }>
	} = {}) {
		...
		const buildLandingEntries = vi.fn().mockResolvedValue(opts.landingEntries ?? [])
		const buildTopicEntries = vi.fn().mockResolvedValue(opts.topicEntries ?? [])
	...
	it('returns 404 for id out of range', async () => {
		...
		// blog count 0 → blogSitemapCount 1 → authorsId 2, landingId 3 → id=999 out of range
		const res = await GET(makeReq(), { params: Promise.resolve({ file: '999.xml' }) })
		expect(res.status).toBe(404)
	})

	it('returns 404 for the id just past topic hubs', async () => {
		const { GET } = await loadRoute({ count: 0 })
		// blog count 0 → landingId 3, topicsId 4, so 5 is the first out-of-range id
		const res = await GET(makeReq(), { params: Promise.resolve({ file: '5.xml' }) })
		expect(res.status).toBe(404)
	})

	it('serves topic hubs for the last id', async () => {
		const { GET, buildTopicEntries, buildLandingEntries } = await loadRoute({
			count: 0,
			topicEntries: [{ url: 'https://example.com/blog/topic/rsu-taxes' }],
		})
		// count 0 → blogSitemapCount 1 → authorsId 2, landingId 3, topicsId 4
		const res = await GET(makeReq(), { params: Promise.resolve({ file: '4.xml' }) })
		expect(res.status).toBe(200)
		expect(buildTopicEntries).toHaveBeenCalledTimes(1)
		expect(buildLandingEntries).not.toHaveBeenCalled()
		const text = await res.text()
		expect(text).toContain('<loc>https://example.com/blog/topic/rsu-taxes</loc>')
	})
	...
	it('serves authors for blogSitemapCount + 1', async () => {
		const { GET, buildAuthorEntries, buildLandingEntries } = await loadRoute({
			count: 0,
	...
	it('serves landing pages for last id', async () => {
		const { GET, buildLandingEntries, buildAuthorEntries } = await loadRoute({
			count: 0,
			landingEntries: [{ url: 'https://example.com/plumber-austin' }],
		})
		// count 0 → blogSitemapCount 1 → authorsId 2, landingId 3
		const res = await GET(makeReq(), { params: Promise.resolve({ file: '3.xml' }) })
		expect(res.status).toBe(200)
		expect(buildLandingEntries).toHaveBeenCalledTimes(1)
		expect(buildAuthorEntries).not.toHaveBeenCalled()
		const text = await res.text()
		expect(text).toContain('<loc>https://example.com/plumber-austin</loc>')
	})

	it('serves landing pages at the shifted id when there are many blog batches', async () => {
		const { GET, buildLandingEntries } = await loadRoute({
			count: 2500,
			landingEntries: [{ url: 'https://example.com/plumber-austin' }],
		})
		// count 2500 → blogSitemapCount 3 → authorsId 4, landingId 5
		const res = await GET(makeReq(), { params: Promise.resolve({ file: '5.xml' }) })
		expect(res.status).toBe(200)
		expect(buildLandingEntries).toHaveBeenCalledTimes(1)
	})
