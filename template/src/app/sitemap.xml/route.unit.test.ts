...
describe('GET /sitemap.xml (index)', () => {
	...
	it('emits static + one blog batch + authors + landing + topics sitemap for small post counts', async () => {
		...
		// blogSitemapCount + 2 = landing pages
		expect(text).toContain('https://example.com/sitemap/3.xml')
		// blogSitemapCount + 3 = topic hubs
	...
	it('emits multiple blog batches plus authors when count exceeds batch size', async () => {
		...
		const res = await GET()
		const text = await res.text()
		// 1 static + 3 blog batches + 1 authors + 1 landing + 1 topics = 7
		const matches = text.match(/<sitemap>/g)
		expect(matches).toHaveLength(7)
		expect(text).toContain('https://example.com/sitemap/4.xml')
		expect(text).toContain('https://example.com/sitemap/5.xml')
		expect(text).toContain('https://example.com/sitemap/6.xml')
	})

	it('still emits static + 1 blog + authors + landing + topics when fetchBlogCount returns 0', async () => {
