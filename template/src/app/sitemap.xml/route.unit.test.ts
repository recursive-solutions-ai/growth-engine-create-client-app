...
describe('GET /sitemap.xml (index)', () => {
	...
	it('emits static + one blog batch + authors + landing sitemap for small post counts', async () => {
	...
	it('emits multiple blog batches plus authors when count exceeds batch size', async () => {
		...
		const res = await GET()
		const text = await res.text()
		// 1 static + 3 blog batches + 1 authors + 1 landing = 6
		const matches = text.match(/<sitemap>/g)
	...
	it('still emits static + 1 blog + authors + landing when fetchBlogCount returns 0', async () => {
