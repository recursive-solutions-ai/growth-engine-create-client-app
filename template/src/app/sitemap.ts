...
const STATIC_PAGES = ['', '/blog', '/blog/authors', '/contact', '/privacy', '/legal', '/cookies']
...
export async function generateSitemaps() {
	...
	// ID 0 = static pages, IDs 1..N = blog batches, ID N+1 = authors
	const ids = [{ id: 0 }]
	for (let i = 1; i <= blogSitemapCount; i++) {
		ids.push({ id: i })
	}
	ids.push({ id: blogSitemapCount + 1 })
	return ids
}
