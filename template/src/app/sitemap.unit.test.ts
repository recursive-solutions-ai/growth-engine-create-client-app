...
describe('sitemap', () => {
	...
	describe('generateSitemaps', () => {
		it('returns static (id=0) plus one blog sitemap (id=1) for small counts', async () => {
			vi.stubGlobal(
				'fetch',
		...
		it('creates multiple blog sitemaps when count exceeds batch size', async () => {
			vi.stubGlobal(
				'fetch',
				vi.fn().mockResolvedValue({
					ok: true,
			...
			// 2500 / 1000 = 3 blog sitemaps + 1 static = 4
			expect(result).toHaveLength(4)
			expect(result).toEqual([{ id: 0 }, { id: 1 }, { id: 2 }, { id: 3 }])
		})

		it('returns static + 1 blog sitemap when fetch fails', async () => {
			vi.stubGlobal(
				'fetch',
		...
		it('returns static + 1 blog sitemap when API returns non-ok response', async () => {
			vi.stubGlobal(
				'fetch',
