...
async function fetchLandingPages(): Promise<LandingSitemapPage[]> {
	try {
		const res = await fetch(`${SITE_URL}/api/rs/content?type=landing_page&limit=1000`, {
			next: { revalidate: 3600 },
		})
		if (!res.ok) return []
		const pages = (await res.json()) as LandingSitemapPage[]
...
/**
 * Published landing pages (service-in-city money pages), which live at the site
 * root. Priority 0.9 — above a blog post and below the homepage, because for a
 * local business these are usually the highest-converting pages on the site.
 *
 * They carry NO hreflang alternates: a landing page is written once, for one
 * place, and is not translated the way a blog post is.
 */
export async function buildLandingEntries(): Promise<SitemapEntry[]> {
	const pages = await fetchLandingPages()
	return pages
		.filter((p) => typeof p.slug === 'string' && p.slug.length > 0)
		.map((p) => ({
			url: buildUrl(`/${p.slug}`, defaultLocale),
...
/**
 * Topic hub pages (`/blog/topic/<slug>`), one set per locale because a hub is
 * built from the posts of one language. Priority 0.7: a pillar page that links
 * to every post on a subject is worth more to a crawler than an author page
 * and less than the posts themselves.
 */
