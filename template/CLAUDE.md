...
| `src/app/[locale]/layout.tsx` | Locale layout: `DictionaryProvider`, `Header`, `Footer` |
...
| `src/i18n/client.tsx` | `DictionaryProvider` + `useI18n()` hook |
| `src/i18n/index.ts` | `getDictionary()` server function with caching |
...
t('blog.load.error', { error: msg })   // with variable interpolation ({varName} syntax)
```
...
**New page:** Create `src/app/[locale]/your-page/page.tsx` with `'use client'` directive. Use `useI18n()` for translations. Add dictionary keys to all locale files.
