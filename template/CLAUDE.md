...
- Dark mode via `data-theme="dark"` attribute on `<html>`, set by the `ThemeToggle` component in the footer
...
| `src/app/[locale]/blog/page.tsx` | Blog listing with search + pagination (page 1) |
| `src/app/[locale]/blog/page/[pageNumber]/page.tsx` | Paginated blog listing (`/blog/page/2`…) — page 1 lives at `/blog` (`/blog/page/1` 308s there), out-of-range/non-numeric pages 404 |
| `src/app/[locale]/blog/[slug]/page.tsx` | Blog detail: `BlogContent` + `TopicChips` ("Filed under") + `RelatedArticles` (3–5 deterministic related posts + booking CTA) |
| `src/app/[locale]/blog/topic/[slug]/page.tsx` | Topic hub (pillar) page — every published post sharing one keyword. Computed from stored keywords via `getBlogTopic()`; 404 when fewer than 2 posts share it |
...
| `src/app/[locale]/[slug]/page.tsx` | Published landing page (service-in-city money page) at the site root. LAST route Next tries, so every static segment wins over it; `notFound()` when no **published** page owns the slug. Drafts never render. |
...
| `src/app/[locale]/newsletter/page.tsx` | Newsletter archive (sent issues) + `NewsletterSignup` |
| `src/app/[locale]/newsletter/[slug]/page.tsx` | Newsletter issue detail via `getNewsletterIssue()` + `NewsletterContent` |
...
| `src/components/layout/` | Header, Footer (hosts `ThemeToggle`), ThemeToggle, LanguageSwitcher |
| `src/lib/theme.ts` | Theme preference model (`light`/`dark`/`auto`), `resolveTheme()`, and the pre-paint `THEME_INIT_SCRIPT` |
...
| `src/app/sitemap/[file]/route.ts` | Sitemap shards: id 0 = static pages, 1..N = blog batches, N+1 = authors, N+2 = landing pages |
...
| `src/app/robots.ts` | Robots.txt — default allows all + disallows `/api/`, then appends per-bot AI-crawler rules from the tenant's Brain-side policy via `getAiCrawlerRules` (sdk-server), revalidated every 5 min so a portal policy change needs no redeploy |
...
| `subscribeToNewsletter` | `subscribeToNewsletter({ email, name? })` | `{ ok, error? }` — single opt-in, idempotent |
| `useJobStatus` | `useJobStatus(jobId)` | `{ job, loading, error, refresh }` (`job` = `{ jobId, type, status, createdAt }`) |
...
To start a job (blog generation, social sync), call `enqueueJob` / `triggerBlogGen` / `triggerSocialSync` from `@growth-engine/sdk-server` in server code behind your own auth check. `POST /api/rs/jobs` returns 405: a public route must never spend the AI budget on an anonymous request.
...
- `BlogList` handles client-side search filtering + pagination (9 posts per page, from `src/lib/blog-pagination.ts`). Pagination is **URL-driven and crawlable**: the blog pages pass `page`, `postsPerPage` + `paginationBasePath`, so pagination renders as plain `<a href>` links (`/blog` → `/blog/page/2` → …) and posts beyond page 1 are never orphaned for crawlers. Never replace these with JS-only buttons — a search-filtered list is the one place client-side (button) pagination is used, since search is inherently a JS interaction.
- `BlogContent` renders HTML via `dangerouslySetInnerHTML` with `prose prose-lg` classes
- `RelatedPosts` shows up to 3 posts excluding the current one (legacy; the template now uses `RelatedArticles`)
- Blog detail page uses `useParams()` to get `slug` and `locale`

## Structural internal linking (related articles, topic hubs, booking CTA)

Links between posts are STRUCTURAL, not editorial: every post links out to 3–5 siblings and up to its pillar page(s) in server-rendered HTML, so no post depends on a writer remembering to link it. All three pieces are computed from stored data by `@growth-engine/types/blog-linking` — the same code the seo-improvement loop runs when it counts a post's inbound links, so never re-rank by hand in a template.

- **`getRelatedPosts(db, post, { locale })`** → 3–5 published posts in the same language, ranked by shared `keywords` (weight 3) then shared significant title words (weight 1), newest posts filling the block when few match. Rendered by `<RelatedArticles posts cta ctaLabel ctaDescription />`.
- **`getBookingCallToAction(db)`** → where the "Book a call" button goes: the owner's own booking link from Settings > Business, else the active form whose name says booking, else any active form, else `tel:` business phone, else `mailto:`. Null hides the slot (a button that leads nowhere is worse than none), and it is ALSO null when the owner switched the booking box off. When the owner chose their own wording it carries `buttonLabel` and `lead`; pass `ctaLabel={bookingCta?.buttonLabel ?? dict['blog.cta.book.call']}` (and the same for `lead`) so their words beat the dictionary default. Never put "free" in the dictionary default: the booking link may be a paid consultation.
- **`getBlogTopics(db, locale)`** → every keyword shared by ≥2 published posts, as `{ slug, label, postSlugs, postCount }`, largest first. `<TopicChips>` renders them on `/blog` (all hubs, with counts) and on each post (`getBlogTopicsForPost`, "Filed under", no counts).
- **`getBlogTopic(db, slug, locale)`** → one hub with its posts, for `/blog/topic/[slug]`. Hubs are in the sitemap (`buildTopicEntries`, the last sitemap file, per locale) and linked from the index, so every hub is crawlable and every post is one click from its cluster.
- Topic slugs come from `topicSlug(keyword)` (lowercase, punctuation dropped, hyphens). "RSU taxes", "rsu-taxes" and "RSU Taxes" are ONE hub — the label is the casing the posts use most.
- The SDK content route serves `?type=blog-topics&locale=xx` (list) and `&slug=` (one hub) for the sitemap and any client-side use.
...
- **Signup:** `<NewsletterSignup />` from `@growth-engine/sdk-client/components` collects an email (and optionally a name via `collectName`) and calls `subscribeToNewsletter({ email, name? })`. Single opt-in, idempotent, deduped by lowercased email; re-subscribing an unsubscribed address flips it back. Props: `title`, `description`, `buttonLabel`, `placeholder`, `namePlaceholder`, `collectName`, `successMessage`, `className`, `onSuccess`. Drop it anywhere (it's a client component).
- **Archive:** `/newsletter` lists published (= sent) issues via `getNewsletterIssues(getDb(), { limit: 0 })` and links each to `/newsletter/{slug}`. `/newsletter/{slug}` renders one issue with `<NewsletterContent issue={...} />` (markdown → `prose`, same as blog) via `getNewsletterIssue(getDb(), slug)` → `notFound()` when null. Both pages carry a `<NewsletterSignup />`.
- **SSR queries (sdk-server):** `getNewsletterIssues(db, { limit?, offset? })`, `getNewsletterIssue(db, slug)`, `getNewsletterIssueCount(db)` — all return the public-safe `PublicNewsletterIssue` shape (`id, slug, subject, previewText, content, sentAt`) and only ever expose `status='sent'` issues. `/newsletter` is in `STATIC_PAGES` (sitemap).

## Landing pages (service-in-city money pages)

Commercial pages the platform generates for high-intent local searches — "birthday party venue san marino", "emergency plumber arcadia". They are written in the portal (or by the seo-improvement loop) and saved as **drafts**; the business owner approves each one before it goes live.

- **Route:** `/{slug}` at the site root, e.g. `/birthday-party-venue-san-marino`. Deliberately not under a `/services/` prefix — the prefix adds nothing for the reader and dilutes the URL.
- **SSR query:** `getLandingPage(getDb(), slug)` and `getLandingPages(getDb(), { limit })` from `@growth-engine/sdk-server`. **Both filter on `status = 'published'`** and there is no flag to include drafts. Do not add one: an unapproved page must never be reachable.
- **Rendering:** `<LandingPageContent page={page} business={business} canonicalUrl={...} />` from `@growth-engine/sdk-client/components`. It renders the markdown body and emits LocalBusiness + FAQPage + BreadcrumbList JSON-LD. The LocalBusiness node appears only when the business config carries an address, and the FAQPage only when the body genuinely has question headings with answers — neither is fabricated.
- **Sitemap:** shard `N+2` (`buildLandingEntries` in `src/lib/sitemap-shared.ts`), priority 0.9, no hreflang alternates (a landing page is written once, for one place).
- **Catch-all caution:** `[locale]/[slug]` matches any single-segment path nothing else claims. When you add a new top-level static route, add its directory under `src/app/[locale]/` as usual — the static segment always wins — and add it to `STATIC_PAGES`.
...
- **Never import non-component VALUES from `@growth-engine/sdk-client/components` in a server component.** The published bundle is marked `"use client"`, so a constant imported into a server page silently becomes a client-reference proxy — arithmetic with it is `NaN` and comparisons are false (this once broke the `/blog/page/N` out-of-range 404). Rendering the *components* from a server page is fine; sharing *values* is not. Server-side constants live in `src/lib/` (e.g. `blog-pagination.ts`) and are passed to SDK components as props.
...
- **Theme is a `data-theme` attribute on `<html>`, resolved in JS.** `ThemeToggle` (footer) offers Light / Dark / Auto; `auto` is the default and follows `prefers-color-scheme`, an explicit click is persisted in `localStorage` under `theme` and wins from then on. The pre-paint `THEME_INIT_SCRIPT` in `src/app/layout.tsx` sets the attribute before first render (hence `suppressHydrationWarning` on `<html>`) — never remove it or dark-preference visitors get a white flash. Resolution logic lives in `src/lib/theme.ts` and is unit-tested.
...
- **Canonical + title:** every page's `generateMetadata` returns `buildPageMetadata({ path, locale, title, description })` ([src/lib/seo.ts](src/lib/seo.ts)). It emits a self-referencing canonical (`buildUrl(path, locale)`), a unique title, hreflang `alternates.languages` (+ `x-default`) in multi-lang mode, and OpenGraph/Twitter tags. The path is the **locale-agnostic** path (`''`, `/blog`, `/blog/${slug}`).
- **Brand suffix — never on `/blog`:** `buildPageMetadata` appends ` | {{CLIENT_NAME}}` only when the branded title still fits in 60 chars AND the path is not under `/blog`. Blog articles, the blog listing and author pages are always unbranded: the company name eats SERP characters, makes every article look like a near-duplicate, and competes with the brand's own homepage result. This is a blanket path rule in `isBlogPath` ([src/lib/seo.ts](src/lib/seo.ts)) — do not "fix" it with a per-page `brand` flag, and never re-add the brand to a blog title by hand. Covered by [src/lib/seo.unit.test.ts](src/lib/seo.unit.test.ts). The Brain-side AI prompts match this rule by keeping the brand name out of generated `seoTitle`s.
