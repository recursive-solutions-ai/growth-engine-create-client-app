...
- **`getBookingCallToAction(db)`** → where "Book a free 15-minute discovery call" goes: the active form whose name says booking, else any active form, else `tel:` business phone, else `mailto:`. Null hides the slot — a button that leads nowhere is worse than none. Pass it as `cta` to `<RelatedArticles>`.
