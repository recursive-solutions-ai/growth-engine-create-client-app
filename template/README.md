...
| `useJobStatus(jobId)` | Polls job progress (status summary only) |

Jobs are never started from the browser. To trigger blog generation or a social sync, call `triggerBlogGen` / `triggerSocialSync` / `enqueueJob` from `@growth-engine/sdk-server` in server code behind your own auth.
